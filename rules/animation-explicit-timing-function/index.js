const stylelint = require('stylelint');
const { pluginNamespace } = require('../../utils/plugin-namespace');
const { unknownErrorOccurredRuleMessage } = require('../../utils/unknown-error-occurred-rule-message');
const { isProjectBemBlockCssFile } = require('../../utils/is-project-bem-block-css-file');
const { isGlobalStylesCssFile } = require('../../utils/is-global-styles-css-file');

const { report, ruleMessages } = stylelint.utils;
const ruleName = `${pluginNamespace}/animation-explicit-timing-function`;

const messages = ruleMessages(ruleName, {
  unexpectedNoTimingFunction: () => 'Unexpected animation without explicit timing function',
  unknownErrorOccurred: unknownErrorOccurredRuleMessage,
});

const TIMING_FUNCTIONS = ['linear'/* 'ease', 'ease-in', 'ease-out', 'ease-in-out', 'step-start', 'step-end' */];

const ruleFunction = () => (root, result) => {
  const cssFullFilePath = root.source?.input?.file;

  if (!isProjectBemBlockCssFile(cssFullFilePath) && !isGlobalStylesCssFile(cssFullFilePath)) {
    return;
  }

  root.walkDecls((decl) => {
    try {
      if (decl.prop?.toLowerCase() === 'animation') {
        // eslint-disable-next-line max-len
        // const { parse } = require('@hookun/parse-animation-shorthand'); -- published as 'Apache License, Version 2.0' and I don't want to use it
        const timingFunction = TIMING_FUNCTIONS.find((item) => decl.value?.toLowerCase().includes(item));
        if (!timingFunction) {
          report({
            ruleName, result, message: messages.unexpectedNoTimingFunction(), node: decl,
          });
        }
      } else if (decl.prop?.toLowerCase() === 'animation-name') {
        let timingFunction;
        decl.parent?.walkDecls((parentDecl) => {
          const parentProperty = parentDecl.prop;
          if (parentProperty === 'animation-timing-function') {
            timingFunction = parentDecl.value;
          }
        });
        if (!timingFunction) {
          report({
            ruleName, result, message: messages.unexpectedNoTimingFunction(), node: decl,
          });
        }
      }
    } catch (e) {
      /* istanbul ignore next */
      report({
        ruleName, result, message: messages.unknownErrorOccurred(e), node: decl,
      });
    }
  });
};

ruleFunction.ruleName = ruleName;
ruleFunction.messages = messages;

module.exports = stylelint.createPlugin(ruleName, ruleFunction);
